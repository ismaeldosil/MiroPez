...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Lightbulb - insight
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Globe - panoramic search
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // Users - conversation
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - 匹配"相关预测事实: X条"格式
    ...
    // Extract sub-queries - 完整提取，不限制数量
    ...
    // Extract core entities - 完整提取，包含摘要和相关事实数
    ...
    // Extract relation chains - 完整提取，不限制数量
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - 完整提取，不限制数量
    ...
    // Extract historical/过期事实 - 完整提取，不限制数量
    ...
    // Extract involved entities - 完整提取，不限制数量
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interviewee count（如 "5 / 9 位模拟Agent"）
    ...
    // Parse each person selection reason
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract answers - 分Twitter和Reddit
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - 完整提取，不限制数量
  ...
  setup(props) {
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        // Empty state
        activeTab.value === 'facts' && props.result.facts.length === 0 && h('div', { class: 'empty-state' }, 'None availableCurrent Key Memories'),
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'None availableCore Entities'),
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'None availableRelation Chains')
      ])
    ])
  }
}
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'None availableCurrent Active Memories'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'None availableHistorical Memories'),
        ...
        activeTab.value === 'entities' && h('div', { class: 'entities-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'None availableInvolved Entities'),
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons（仅在有真实双平台回答时显示）
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title（## xxx），since section title is already shown outside
  ...
  // Process line breaks - 空行变成段落分隔，单换行变成 <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Clean before/after block elements <br> 标签
  html = html.replace(/<br>\s*(<ul|<ol|<blockquote)/g, '$1')
  html = html.replace(/(<\/ul>|<\/ol>|<\/blockquote>)\s*<br>/g, '$1')
  // 清理 <p><br> 紧跟块级元素的情况（多余空行导致）
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> 标签
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // 清理块级元素后紧跟的段落开始标签前的 <br>
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering：当单项 <ol> 被段落内容隔开时，保持编号递增
...
const extractFinalContent = (response) => {
  ...
  // Try to extract <final_answer> 标签内的内容
