...
// 格式化Simulation elapsed time（根据rounds次和minutes per round计算）
...
// Track each platform last round to detect changes and log
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check if simulation is completed（通过 runner_status 或平台完成状态判断）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check：如果后端还没来得及更新 runner_status，但平台已经报告完成
