    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            结合上下文，自动调用工具从知识图谱梳理实体与Relations，Initialization模拟个体，并基于现实种子赋予他们独特的行为与记忆
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM 根据Simulation Requirement与现实种子，智能设置世界时间流速、推荐算法、每个个体的活跃时间段、发言频率、事件触发等参数
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">每hours活跃</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            基于Narrative direction，自动生成初始激活事件与热点话题，引导模拟世界的初始状态
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">模拟环境已Preparation Complete，可以开始运行模拟</p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish 自动规划推演现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours，每rounds代表现实 <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes时间流逝</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">若首次运行，强烈建议切换至‘Custom模式’减少模拟rounds数，以便快速预览效果并降低报错风险 ➝</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="modal-body">
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                <span class="dim-title">社会Relations网络</span>
...
// State
const phase = ref(0) // 0: Initialization, 1: 生成人设, 2: 生成配置, 3: 完成
...
// Log deduplication：记录上一次输出的关键信息
...
watch(currentStage, (newStage) => {
  if (newStage === '生成Agent persona' || newStage === 'generating_profiles') {
    phase.value = 1
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return when config not generated null
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Return when config data incomplete null
  }
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(`Start Simulation，Customrounds数: ${customMaxRounds.value} rounds`)
  } else {
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 立即设置Expected Total Agents（Get from prepare API response）
      ...
      addLog(`Preparation failed: ${res.error || 'Unknown错误'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        addLog(`✗ Preparation failed: ${data.error || 'Unknown错误'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update when API returns valid value，避免覆盖已有的有效值
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Log config generation phase（避免重复）
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('GeneratingAgent persona配置...')
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time config: 每rounds${tc.minutes_per_round}minutes, 共${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}rounds`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Env Setup完成，可以Start Simulation')
...
const loadPreparedData = async () => {
  ...
  addLog(`已加载 ${profiles.value.length} 个Agent persona`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // 配置尚未生成，开始rounds询
        addLog('配置Generating，开始rounds询等待...')
        startConfigPolling()
      }
    }
  } catch (err) {
...
onMounted(() => {
  // Auto-start preparation流程
  if (props.simulationId) {
    addLog('Step2 Env SetupInitialization')
