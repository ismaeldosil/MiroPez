...
const renderGraph = () => {
  ...
  // Process edge data，计算同一对Nodes间的边数量和索引
  const edgePairCount = {}
  const selfLoopEdges = {} // Grouped by node自环边
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count each pairNodes之间的边数量，收集自环边
  tempEdges.forEach(e => {
  ...
  // Record current pairNodes的第几条边
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Already processed自环Nodes
  ...
  tempEdges.forEach(e => {
    ...
    // Determine edge direction是否与标准化方向一致（源UUID < 目标UUID）
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature：多条边时分散开，单条边为直线
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute曲率，确保明显区分
      // Curvature range based on边数量增加，边越多曲率范围越大
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If edge direction is reversed, flip curvature
      // Ensure all edges in same reference frame下分布，不会因方向不同而重叠
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add central gravity，让独立的Nodes群聚集到中心区域
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop：绘制一个圆弧从Nodes出发再返回
      const loopRadius = 30
      // Start from right side of node，绕一圈回来
      const x1 = sx + 8  // Start offset
      const y1 = sy - 4
      const x2 = sx + 8  // End offset
      const y2 = sy + 4
      // Draw using arc自环（sweep-flag=1 顺时针）
    ...
    // Calculate curve control point - 根据边数量和距离动态调整
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular to link direction的偏移，根据距离比例计算，保证曲线明显可见
    // 边越多，偏移量占距离的比例越大
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bezier curve的中点 t=0.5
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    const pairTotal = d.pairTotal || 1
    ...
    // Quadratic Bezier curve公式 B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Reset previously selected edge的样式
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#666')
      // Highlight current selection的边
  ...
  // Save ref for external control显隐
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position，不重启仿真（区分点击和拖拽）
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect if actually dragging（移动超过阈值）
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // First real drag detected，才重启仿真
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only if actually dragged才让仿真逐渐停止
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions（无旋转，水平显示更清晰）
