        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成的Relations类型 ({{ projectData.ontology.relation_types?.length || 0 }})</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Description</div>
                <div class="detail-content">
                  基于生成的本体，将文档分块后调用 Zep API 构建知识图谱，提取实体和Relations
                </div>
              </div>
...
// Current project ID（可能从'new'变为实际ID）
...
// Handle new project - call ontology API
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = '项目Initialization失败: ' + (err.message || 'Unknown错误')
  } finally {
    loading.value = false
  }
}

// Load existing project数据
const loadProject = async () => {
  try {
    ...
    error.value = 'Failed to load project: ' + (err.message || 'Unknown错误')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    error.value = '启动Graph Build失败: ' + (err.message || 'Unknown错误')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Update progress display为完成状态
        buildProgress.value = {
          progress: 100,
          message: 'Build Complete，正在加载图谱...'
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Graph Build失败: ' + (task.error || 'Unknown错误')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  // Create node ID set for filtering有效边
  ...
  simulation.on('tick', () => {
    // Update all edge positions（包括可见线和透明点击区域）
