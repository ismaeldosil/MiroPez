...
class OasisProfileGenerator:
    ...
    Converts entities from the Zep graph into Agent Profiles required by the OASIS simulation
    ...
    # Group/institution entity types (require representative account persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to get rich information related to an entity

        Zep does not have a built-in hybrid search API, so edges and nodes are
        searched separately and results are merged. Uses parallel requests for efficiency.
    ...
    def _generate_profile_with_llm(
        ...
        # Try generating multiple times until success or max retry count
        max_attempts = 3
        last_error = None
...
- 机构账号发言要符合其身份定位"""
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profiles in CSV format (following OASIS official requirements)
